...
async function validateExplanation(text, inputData) {
  ...
  if (!match) {
    ...
    return {
      score: 0,
      feedback:
        "Interner Fehler: Validator-Antwort konnte nicht ausgewertet werden.",
    };
  }
  ...
  try {
    ...
    return {
      score: 0,
      feedback: "Interner Fehler: Validator-Antwort ist ungültig.",
    };
  }
}
...
app.post("/ai/explain", async (req, res) => {
  try {
    const { diagnosis, icd10Code, year, status, entryBy, comment, langLevel } =
      req.body;
    ...
    while (attempts < MAX_ATTEMPTS) {
      ...
      console.log(
        `▶️ Versuch ${attempts}/${MAX_ATTEMPTS} – Hauptmodell generiert...`,
      );
    ...
    res.json({
      text: responseText,
      disclaimer:
        "Dies ist eine KI-generierte Erklärung - kein Ersatz für ein persönliches Arztgespräch.",
    });
  } catch (error) {
...
const diagnosisModelWithFields =
  diagnosisModel.withStructuredOutput(DiagnosisFields);
...
const medicationModelWithFields =
  explainModel.withStructuredOutput(MedicationFields);
